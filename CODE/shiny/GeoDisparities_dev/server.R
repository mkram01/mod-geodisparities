...
shinyServer(function(input, output, session) {
  ...
  output$contextvar <- renderUI(selectInput('contextvar',label='Contextual Variable',
                                         choices = names(contextdf)[!names(contextdf) %in% c('GEOID','year', 'dataname', 'state_name','county_name', 'region.ID', 'NAME', 'geometry')],
                                         selected =  names(contextdf)[!names(contextdf) %in% c('GEOID','year', 'dataname', 'state_name','county_name', 'region.ID', 'NAME', 'geometry')][1]))
  output$modvar <- renderUI(selectInput('modvar',label='Perinatal Outcome Variable',
                                          choices = names(moddf)[!names(moddf) %in% c('GEOID', 'year', 'dataname', 'state_name', 'county_name', 'NAME', 'region.ID', 'geometry')],
                                          selected =  names(moddf)[!names(moddf) %in% c('GEOID', 'year', 'dataname', 'state_name', 'county_name', 'NAME', 'region.ID', 'geometry')][2]))
  
  # Defining xvar, yvar & color var reactives -------------------------------------------------------
  xvar_ <- ''
  xVar <- reactive({
    print('reactive: xVar')
    if(is.null(input$contextvar)) return(names(contextdf)[!names(contextdf) %in% c('GEOID','year', 'dataname', 'state_name','county_name', 'region.ID', 'NAME', 'geometry')][1])
    xvar_ <<- input$contextvar
    input$contextvar})
  yVar <- reactive({
    print('reactive: yVar')
    if(is.null(input$modvar)) return(names(moddf)[!names(moddf) %in% c('GEOID', 'year', 'dataname', 'state_name', 'county_name', 'NAME', 'region.ID', 'geometry')][2])
    input$modvar})
