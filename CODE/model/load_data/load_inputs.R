...
######################################################################################################
message("From load_inputs.R script: Loading aspatial data and prepping for model.")
...
if (file.exists(paste0(data_repo, "/model_input/adjacency_matrices/", adjfilename)) & create_spwts == FALSE){
  message(paste0("From load_inputs.R script: The adjacency matrix for your geography already exists and you have elected not to recreate it. Loading it now!"))
  #load adjacency matrix
  model_spwts <- inla.read.graph(paste0(data_repo, "/model_input/adjacency_matrices/", adjfilename))
} else {
  message(paste0("From load_inputs.R script: Either the adjacency matrix for your geography does not exist or you have elected to recreate it. Creating it now!"))
...
message("From load_inputs.R script: Finished loading and prepping data!")
